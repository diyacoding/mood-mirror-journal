        ...
        <div className="h-44 -mx-2">
          <ResponsiveContainer>
            <LineChart data={last30} margin={{ top: 8, right: 8, bottom: 0, left: -20 }}>
              ...
              <Tooltip
                contentStyle={{ borderRadius: 16, border: "1px solid hsl(var(--glow) / 0.3)", background: "hsl(var(--card) / 0.95)", backdropFilter: "blur(12px)", color: "hsl(var(--card-foreground))" }}
                labelStyle={{ color: "hsl(var(--accent))" }}
                formatter={(v: any) => [v?.toFixed?.(1) ?? "—", "Mood"]}
              />
              <Line type="monotone" dataKey="mood" stroke="url(#moodGradient)" strokeWidth={3} dot={{ r: 3, fill: "hsl(var(--glow))", strokeWidth: 0 }} activeDot={{ r: 5, fill: "hsl(var(--glow))" }} connectNulls
                style={{ filter: "drop-shadow(0 0 8px hsl(var(--glow-deep) / 0.7))" }}
              />
            </LineChart>
          </ResponsiveContainer>
        </div>
          ...
          {moodCounts.map((m) => {
            ...
            return (
              <div key={m.key} className="flex items-center gap-3">
                <span className="text-xl w-6 drop-shadow-[0_0_10px_hsl(var(--glow)/0.4)]">{icons.mood(m.key)}</span>
                <div className="flex-1 h-2 rounded-full bg-muted/60 overflow-hidden">
                  <div className="h-full rounded-full transition-smooth" style={{ width: `${pct}%`, background: `linear-gradient(90deg, ${m.color}, hsl(var(--glow)))`, boxShadow: `0 0 12px ${m.color}` }} />
                </div>
